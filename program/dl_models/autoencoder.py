...
TOKENIZER_OUT_PATH = "../../data/tokenizer_cs"
# OUT_FOLDER = "/home/hoanh/devcode/LabRISE/CodeSmell/DeepLearningSmells/results/autoencoder"
OUT_FOLDER = "./results/"
...
def find_optimal(error_df):
    ...
    for threshold in range(1000, 400000, 5000):
        ...
        if f1 > max_f1:
            ...
            max_re = recall
            max_mcc = mcc
    return optimal_threshold, max_pr, max_re, max_f1, max_mcc
    ...
    for layer in layers:
        for bottleneck in [True]:
            for encoding in encoding_dim:
                ...
                try:
                    optimal_threshold, max_pr, max_re, max_f1, max_mcc = autoencoder_lstm(input_data, smell, layers=layer,
                ...
                write_result(outfile,
                             ...
                             + "," + str(layer) + "," +
                             str(max_pr) + "," + str(max_re) + "," + str(max_f1) + "," + str(max_mcc) + "," + str(time_taken) + "\n")
    ...
    for layer in layers:
        for bottleneck in [True]:
            for encoding in encoding_dim:
                ...
                start_time = time.time()
                try:
                    optimal_threshold, max_pr, max_re, max_f1, max_mcc = autoencoder_dense(input_data, smell, layers=layer,
                                                                                  epochs=epochs,
                                                                                  # batch_size=batch_size,
                                                                                  encoding_dimension=encoding,
                                                                                  with_bottleneck=bottleneck)
...
def main_cnn(smell, data_path, max_encoding_dim=1024):
    ...
    write_result(outfile, "conv_layers,filters,kernel,max_pooling_window,epochs,precision,recall,f1,mcc,time\n")
    for layer in layers:
        for filter in filters:
            for kernel in kernels:
                for pooling_window in pooling_windows:
                    ...
                    start_time = time.time()
                    try:
                        optimal_threshold, max_pr, max_re, max_f1, max_mcc = autoencoder_cnn(input_data, config)
                    except ValueError as error:
                        print(error)
                        optimal_threshold = -1
                        max_pr = -1
